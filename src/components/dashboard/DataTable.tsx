...
function DataTableInner({ planets }: Props) {
  ...
  const tdCls =
    "px-3 py-2 text-xs text-[var(--foreground)] whitespace-nowrap";
          ...
          <AnimatePresence mode="wait">
            ...
            >
              {slice.map((planet) => (
                <tr
                  key={planet.pl_name}
                  className="border-b border-[var(--border)] border-opacity-40 hover:bg-white/[0.03] transition-colors"
                >
                  {COLUMNS.map((col) => {
...
// Memoize — re-renders only when the planets array reference changes
